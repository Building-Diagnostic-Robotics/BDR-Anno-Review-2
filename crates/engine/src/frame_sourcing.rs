...
{
    extract_frames_from_mp4_with_progress_and_cancel(options, on_progress, || false)
}

pub fn extract_frames_from_mp4_with_progress_and_cancel<F, C>(
    options: ExtractFramesFromMp4Options,
    mut on_progress: F,
    should_cancel: C,
) -> Result<ExtractFramesFromMp4Report, EngineError>
    ...
    C: Fn() -> bool,
{
    ...
    let skipped_existing_count = extract_frames_batch(
        ffmpeg_bin,
        &mp4_path,
        &plan.mappings,
    ...
    C: Fn() -> bool,
{
    ...
    for mapping in mappings {
        ...
        pending.push((mapping.frame_index, output_path));
    }

    if pending.is_empty() {
        return Ok(skipped_existing_count);
    }
...
"#,
        ...
        let report = extract_frames_from_mp4(ExtractFramesFromMp4Options {
            dataset_root: options.dataset_root.clone(),
            coco_json_path: options.coco_json_path.clone(),
        ...
        assert_eq!(report.extracted_frame_count, 2);
        assert_eq!(report.skipped_existing_count, 1);
